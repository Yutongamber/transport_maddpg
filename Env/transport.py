...
from Examples.transport_7plus.Env.gridgame import GridGame
...
#         #print for test
    ...
    def reset(self):
        ...
        for m in range(self.materials): # 3 * 4 = 12 + 121 = 133
    ...
    def get_next_state(self, joint_action):
        ...
        for i in range(self.cars):
            ...
            if (check):
                ...
                if (self.car_map[old_x][old_y] == 0 and self.map[old_x][old_y] == 4):
    ...
    def get_observation_s(self, s):
        ...
        for i in range(self.materials):
            state_id = state_id + [self.material_points[i].position[0], self.material_points[i].position[1], self.material_points[i].number]
        for i in range(self.cars):
            state_id = state_id+[self.cars_list[i].number, self.cars_list[i].position[0], self.cars_list[i].position[1]]
        return state_id

    def is_terminal(self):
        return self.step_cnt > self.max_step - 1
